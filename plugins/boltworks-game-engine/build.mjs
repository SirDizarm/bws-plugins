...
const result=await build({entryPoints:[resolve(root,'src/exporter.js')],bundle:true,format:'iife',minify:true,target:'es2020',write:false,nodePaths:[resolve(process.env.BWS_PLUGIN_DEPENDENCIES||root,'node_modules')]});
...
console.log('Built BoltWorks Game Engine '+manifest.version+' ('+Buffer.byteLength(text)+' bytes)');
